...
- One file per slug, `kb/charts/<slug>.md`, frontmatter `slug` equals the file name. Every target in `kb/targets/` gets a `support:` line (`native`, `approx`, `image`, `none`) and every `native`/`approx` target gets a `### <target>` subsection under `## Build`.
